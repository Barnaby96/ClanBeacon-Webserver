...
def notify_tile_correction(team_id, tile_id):
    team_data = database.get_team_by_id(
        team_id
    )
    ...
    message = (
        f"Correction for {team.team_name}: "
        f"{tile.tile_name} at {tile.board_coordinate} "
...
def notify_tile_corrections(reopened_tiles):
    ...
    for reopened_tile in reopened_tiles:
        ...
        if correction_key in notified_corrections:
            continue

        notified_corrections.add(
            correction_key
        )

        try:
            notify_tile_correction(
                team_id=correction_key[0],
                tile_id=correction_key[1]
            )
        except Exception:
            logger.exception(
                "Failed to send tile correction notification "
                "for team_id=%s tile_id=%s.",
                correction_key[0],
                correction_key[1]
            )
